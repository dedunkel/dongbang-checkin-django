...
def _account_row(user):
    # _tier_of()가 user.user_permissions.all()을 순회하는데, accounts_dashboard가
    # 이미 content_type까지 Prefetch해둔 덕분에 계정 수만큼 쿼리가 나가는
    # N+1 없이(한 번에) 여기서 계산된다.
    tier = _tier_of(user)
...
# 계정 관리 화면은 슈퍼유저 전용 — 계정을 만들고 개별 권한을 배정하는
# 화면이라, 운영진에게까지 열어두면 스스로 슈퍼유저 계정을 만들 수 있게
# 되어버린다 (staff_member_required는 is_staff만 확인하므로 그 위에
...
def accounts_dashboard(request):
    ...
    User = get_user_model()
    users = User.objects.filter(is_staff=True).prefetch_related(
