...
1. 그 점수 시트 파일을 열고 확장 프로그램 > Apps Script.
2. `google-apps-script/SheetSync.gs` 내용을 통째로 붙여넣고, `CONFIG`의
   `SECRET`(아무 랜덤 문자열), `GENRE_TAB_MAP`(장르별 탭 이름), `COLS`(실제
   헤더 이름)를 맞춥니다.
3. 배포 > 새 배포 > 유형: 웹 앱, 실행 계정: 나, 액세스 권한: 전체 허용 →
   배포하면 나오는 URL(`/exec`로 끝남)을 확인합니다.
4. 서버 쪽 `.env`에 `SHEET_SYNC_URL`(3번 URL)과 `SHEET_SYNC_SECRET`(2번과
   동일한 값)을 채웁니다. 둘 다 비워두면 이 기능 없이 체크인만 동작합니다.
5. 참가자 이름+연락처로 그 탭에서 행을 찾아 "도착 여부"에 표시하는
   방식이라, 시트의 이름·연락처 값이 실제 참가자 정보와 일치해야 합니다.
   일치하는 행을 못 찾아도 체크인 자체는 정상 처리되고, 시트 반영만
   조용히 실패합니다 (서버 로그에서 확인 가능).
