...
6. **행사 당일**: 스태프가 본인 계정으로 로그인한 상태에서 아이패드/아이폰
   ...
   기기당 로그인은 최초 1회면 충분하도록 세션이 오래(7일) 유지되는데, 그
   기기를 분실·도난당했다면 **즉시 `/admin/accounts/`에서 그 계정의 "계정
   활성화"를 꺼주세요** — 이미 로그인돼 있던 기기도 다음 요청부터 바로
   접근이 끊깁니다(다음 로그인부터 막는 게 아니라 즉시 차단).
